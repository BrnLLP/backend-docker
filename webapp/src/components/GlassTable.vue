  ...
  data() {
    return {
      columns: [
        ...
        { name: 'tipo_vidro', label: 'Tipo', width: 120 },
...
<template>
  <Table :columns="columns" :data="data">
    ...
    <template v-slot:table-actions="{ row }">
      <div class="glass-table__actions">
        <span class="glass-table__icon" @click="$emit('edit', row)">
          <PencilIcon />
        </span>
        <span class="glass-table__icon" @click="$emit('remove', row)">
          <TrashIcon />
        </span>
      </div>
    </template>
