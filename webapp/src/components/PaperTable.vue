  ...
  data() {
    return {
      columns: [
        ...
        { name: 'tipo_papel', label: 'Tipo', width: 120 },
  ...
  <Table :columns="columns" :data="data">
    ...
    <template v-slot:table-actions="{ row }">
      <div class="paper-table__actions">
        <span class="paper-table__icon" @click="$emit('edit', row)">
          <PencilIcon />
        </span>
        <span class="paper-table__icon" @click="$emit('remove', row)">
          <TrashIcon />
        </span>
      </div>
    </template>
